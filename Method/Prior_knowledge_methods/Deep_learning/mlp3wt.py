"""
Deep multilayer perceptron (MLP3) for pulse shape discrimination using Wavelet Transform features.
"""
...
class MLP3WT(nn.Module):
    def __init__(self, input_length):
        ...
        self.fc1 = nn.Linear(self.padded_input_length, 512)
    ...
    def forward(self, x):
        ...
        if x.shape[1] != self.original_input_length:
            raise ValueError(f"Input length mismatch: model expects input length {self.original_input_length}, but got {x.shape[1]})")
...
def train(train_data, train_labels, task, feat_name):
    ...
    model = MLP3WT(input_length).to(device)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            miu, sigma, fom = histogram_fitting_compute_fom(output.cpu(), 'MLP3WT_train', show_plot=True)
    ...
    model_path = f"Output/Trained_models/MLP3WT_{task}.pth" if task == "classification" else f"Output/Trained_models/MLP3WT_{task}_{feat_name}.pth"
...
def test(test_data, task, feat_name):
    ...
    if test_data.shape[1] != model.original_input_length:
        raise ValueError(f"Input length mismatch: model expects input length {model.original_input_length}, but got {test_data.shape[1]})")
    with torch.no_grad():
