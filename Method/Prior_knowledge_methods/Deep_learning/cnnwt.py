"""
CNN for pulse shape discrimination using wavelet transform spectrograms.
"""
import numpy as np
import torch
...
class CNNWTModel(nn.Module):
    def __init__(self, task="classification"):
        """
        Initialize CNN model for wavelet transform spectrograms.
        
        Args:
            task: Task type (classification/regression)
        """
        super(CNNWTModel, self).__init__()
        self.task = task
...
def train(train_signals, train_labels, task, feat_name, batch_size=256):
    ...
    _model = CNNWTModel(task=task).to(device)
    ...
    if task == "classification":
        ...
        mu, sigma, fom = histogram_fitting_compute_fom(outputs.squeeze().cpu().numpy(),
                                                       'CNNWT_train', show_plot=True)
        print(f"Training PSD factors computed. FOM: {fom}")

    # Save model
    os.makedirs('Output/Trained_models', exist_ok=True)
    model_path = (f"Output/Trained_models/CNNWT_{task}.pth" if task == "classification"
                  else f"Output/Trained_models/CNNWT_{task}_{feat_name}.pth")
