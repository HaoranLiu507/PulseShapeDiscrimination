"""
Deep multilayer perceptron (MLP3) for pulse shape discrimination using PCA features.
...
class MLP3PCA(nn.Module):
    def __init__(self, n_features):
        """
        Initialize MLP3PCA model with PCA preprocessing.
        
        Args:
            n_features: Length of input signal
        """
        super(MLP3PCA, self).__init__()
        ...
        # MLP layers starting with 100 input features
...
def train(train_data, train_labels, task, feat_name):
    ...
    model = MLP3PCA(n_features).to(device)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            miu, sigma, fom = histogram_fitting_compute_fom(output.cpu(), 'MLP3PCA_train', show_plot=True)
    ...
    model_path = f"Output/Trained_models/MLP3PCA_{task}.pth" if task == "classification" else f"Output/Trained_models/MLP3PCA_{task}_{feat_name}.pth"
