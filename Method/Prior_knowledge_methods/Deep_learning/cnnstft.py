"""
CNN for pulse shape discrimination using STFT spectrograms.
"""
import numpy as np
import torch
...
def compute_spectrogram(signal, target_size=(224, 224)):
    """
    Convert 1D signal to spectrogram using Short-Time Fourier Transform.
    Returns normalized RGB image of specified size.
    """
    f, t, Zxx = stft(signal, fs=1, window='hann', nperseg=64, noverlap=32)
...
def signals_to_tensor(signal_list, transform, target_size=(224, 224)):
    ...
    import time
    time.sleep(0.1)
    for sig in tqdm(signal_list, desc="Converting signals to STFT spectrogram tensors"):
...
class CNNSTFTModel(nn.Module):
    def __init__(self, task="classification"):
        """
        Initialize CNN model for STFT spectrograms.
        
        Args:
            task: Task type (classification/regression)
        """
        super(CNNSTFTModel, self).__init__()
        self.task = task
...
def train(train_signals, train_labels, task, feat_name, batch_size=256):
    ...
    _model = CNNSTFTModel(task=task).to(device)
    ...
    if task == "classification":
        ...
        mu, sigma, fom = histogram_fitting_compute_fom(outputs.squeeze().cpu().numpy(),
                                                       'CNNSTFT_train', show_plot=True)
        print(f"Training PSD factors computed. FOM: {fom}")

    # Save model
    os.makedirs('Output/Trained_models', exist_ok=True)
    model_path = f"Output/Trained_models/CNNSTFT_{task}.pth" if task == "classification" else f"Output/Trained_models/CNNSTFT_{task}_{feat_name}.pth"
