"""
Deep multilayer perceptron (MLP3) for pulse shape discrimination using Fourier Transform features.
"""
import torch
...
class MLP3FT(nn.Module):
    def __init__(self, input_length):
        """
        Initialize MLP3FT model.
        
        Args:
            input_length: Length of input signal (must be even)
        """
        super(MLP3FT, self).__init__()
        if input_length % 2 != 0:
        ...
        self.fc1 = nn.Linear(freq_length, 512)
...
def train(train_data, train_labels, task, feat_name):
    ...
    model = MLP3FT(input_length).to(device)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            miu, sigma, fom = histogram_fitting_compute_fom(output.cpu(), 'MLP3FT_train', show_plot=True)
    ...
    model_path = f"Output/Trained_models/MLP3FT_{task}.pth" if task == "classification" else f"Output/Trained_models/MLP3FT_{task}_{feat_name}.pth"
