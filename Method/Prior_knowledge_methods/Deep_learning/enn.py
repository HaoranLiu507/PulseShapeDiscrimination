"""
Elman Neural Network (ENN) for pulse shape discrimination.
Features recurrent processing with additional fully connected layers.

Reference:
- Zhang et al. "Discrimination of neutrons and γ-rays in liquid scintillator 
  based on Elman neural network." Chinese physics C 40.8 (2016): 086204.
"""
import torch
...
class ENN(nn.Module):
    def __init__(self, input_length, hidden_size=20, output_size=1):
        """
        Initialize ENN model.
        
        Args:
            input_length: Length of input sequence
            hidden_size: Number of RNN units (default: 20)
            output_size: Size of output layer (default: 1)
        """
        super(ENN, self).__init__()
        
        # Network layers
        self.fc1 = nn.Linear(input_length, 80)  # Input projection
        self.rnn = nn.RNN(input_size=1, hidden_size=hidden_size, batch_first=True)  # Recurrent layer
        self.fc2 = nn.Linear(hidden_size, 15)  # Hidden projection
        self.output = nn.Linear(15, output_size)  # Output layer
        self.input_length = input_length

    def forward(self, x):
        """
        Forward pass: project -> RNN -> hidden -> output.
...
def train(train_data, train_labels, task, feat_name):
    ...
    model = ENN(input_length).to(device)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            miu, sigma, fom = histogram_fitting_compute_fom(output.cpu(), 'ENN_train', show_plot=True)
            print(f"Training PSD factors computed. FOM: {fom}")

    # Save model
    model_path = f"Output/Trained_models/ENN_{task}.pth" if task == "classification" else f"Output/Trained_models/ENN_{task}_{feat_name}.pth"
    save_dict = {
