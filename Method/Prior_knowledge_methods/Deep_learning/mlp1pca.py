"""
Single layer perceptron (MLP1) for pulse shape discrimination using PCA features.
...
class MLP1PCA(nn.Module):
    def __init__(self, n_features):
        """
        Initialize MLP1PCA model with PCA preprocessing.
        
        Args:
            n_features: Length of input signal
        """
        super(MLP1PCA, self).__init__()
    ...
    def forward(self, x):
        """
        Forward pass: PCA -> linear layer -> sigmoid.
...
def train(train_data, train_labels, task, feat_name):
    ...
    model = MLP1PCA(n_features).to(device)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            miu, sigma, fom = histogram_fitting_compute_fom(output.cpu(), 'MLP1PCA_train', show_plot=True)
    ...
    model_path = f"Output/Trained_models/MLP1PCA_{task}.pth" if task == "classification" else f"Output/Trained_models/MLP1PCA_{task}_{feat_name}.pth"
