"""
Deep multilayer perceptron (MLP3) for pulse shape discrimination using Short-Time Fourier Transform features.
"""
import torch
...
class MLP3STFT(nn.Module):
    def __init__(self, input_length):
        """
        Initialize MLP3STFT model.
        
        Args:
            input_length: Length of input signal
        """
        super(MLP3STFT, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.relu(self.fc1(x_trans))
...
def train(train_data, train_labels, task, feat_name):
    ...
    model = MLP3STFT(input_length).to(device)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            miu, sigma, fom = histogram_fitting_compute_fom(output.cpu(), 'MLP3STFT_train', show_plot=True)
    ...
    model_path = f"Output/Trained_models/MLP3STFT_{task}.pth" if task == "classification" else f"Output/Trained_models/MLP3STFT_{task}_{feat_name}.pth"
