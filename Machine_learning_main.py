...
def fix_flipped_labels(predictions, true_labels):
    """
    Fix flipped labels by using majority voting on clusters.
    
    Args:
        predictions: FCM predicted labels (0 or 1) for each sample
        true_labels: True class labels (0 or 1) for each sample
        
    Returns:
        Fixed predictions (corrected labels)
    """
    initial_accuracy = np.mean(predictions == true_labels)
    
    if initial_accuracy < 0.5:
        print("Flipping prediction labels")
...
def main() -> None:
    ...
    BASE_PATH = 'Method.Prior_knowledge_methods.Machine_learning'
    ...
    available_psd_methods = [
        ...
        'LINRE  - Linear Regression',
        'LOGRE  - Logistic Regression',
        'LRSTFT - Logistic Regression with Short Time Fourier Transform',
    ...
    psd_method_modules = {
        ...
        'GMM': f'{BASE_PATH}.gmm',
        'KNN': f'{BASE_PATH}.knn',
        'LINRE': f'{BASE_PATH}.linre',
        'LOGRE': f'{BASE_PATH}.logre',
        'LRSTFT': f'{BASE_PATH}.lrstft',
        'LVQ': f'{BASE_PATH}.lvq',
        'SVM': f'{BASE_PATH}.svm',
        'TEM': f'{BASE_PATH}.tempotron',
    }
    ...
    # Choose task
    print('Choose the task: classification or regression:')
    ...
    if choice == 'yes':
        ...
        if task == 'classification':
            ...
            if nan_count > 0:
                ...
                print(f"Warning: {nan_count} signals ({nan_percentage:.4f}% of total {total_signals} signals) have NaN label values.")
    ...
    if task == 'classification':
        ...
        # Many machine learning clustering methods are unsupervised, meaning they can differentiate 
        # between two signal classes but are unable to assign the corresponding class labels based 
        # on the ground truth. As a result, we compare the predictions with the ground truth and correct 
        # any flipped labels using majority voting on the clusters.
        fixed_predictions = fix_flipped_labels(Test_pred, Test_labels)
    ...
    if choice == 'yes':
        ...
        print(f"Validation process took {validation_end - validation_start:.2f} seconds.")
